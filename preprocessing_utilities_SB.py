...
"""Utilities used for preprocessing of ........"""
...
'''Not included in tensorflow version 2.90

{'ConvNeXtTiny': (keras.applications.ConvNeXtTiny, "None"),
'ConvNeXtSmall': (keras.applications.ConvNeXtSmall, "None"),
'ConvNeXtBase': (keras.applications.ConvNeXtBase, "None"),
'ConvNeXtLarge': (keras.applications.ConvNeXtLarge, "None"),
'ConvNeXtXLarge': (keras.applications.ConvNeXtXLarge, "None")}'''

'''applications with input shape errors or warnings
...
def features_for_array(x_array: NDArray, batch_size: int = 30, pbar_desc: str = "") \
    ...
    Args:
        x_array (NDArray): The data from which to generate features.  Should have shape
            (N, 480, 480, 3) and integer values in range 0 to 255.  The latter condition will
            not be checked.
    ...
    Raises:
        ValueError: Occurs when shape of `x_array` is not (N, 480, 480, 3).
    """

    if not np.allclose(x_array.shape[1::], (480, 480, 3)):
...
def preprocess_rgba_array(rgba_array: NDArray, augment: bool = False,
    ...
    This function will:
        ...
        2. Pad the image to 480*3 by 480*3 (to allow rotations)
        3. Perform random flipping and rotation (if `augment`=True)
        4. Downsample the image back to 480 by 480.
    ...
    Returns:
        NDArray: The processed image with shape (480, 480, 3)
    """
    ...
    # Pad to 480 * 4
    output_array = pad_rgb_array(output_array, (480 * 4, 480 * 4))

    # Downsample to 480 x 480
    output_array = downsample_rgb_array(output_array, 4)

    # Augment if requested
    if augment:
        # Random rotate (suppress because currently enables rotation from 0 - 2pi)
    ...
    #output_array = keras_normalization(output_array[None, ...], normalization_kind) #if need it to be 4 dims
    #output_array = output_array[0]
    output_array = keras_normalization(output_array, normalization_kind)
